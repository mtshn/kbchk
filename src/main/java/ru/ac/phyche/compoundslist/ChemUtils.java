...
/**
 * 
 * A wrapper class of Chemistry Development Kit (CDK). Based on code from our
 * previous work: https://www.doi.org/10.6084/m9.figshare.12651680
 * https://doi.org/10.1109/ACCESS.2020.3045047 Basic chemoinformatics routines.
 * CDK 2.7.1 is supported; Note, that pretrained models are extremely sensitive
 * to version of CDK and any changes in this class (because "canonical" order of
 * atoms in a SMILES string depends on CDK version, as well as 2D-layout and
 * descriptor values in some cases).
 *
 */
public class ChemUtils {
	 ...
	 */
	public static final int SMILES_LEN = 300;
	/**
	...
	public static enum FingerprintsType {
		...
		ADDITIVE_CIRCULAR_4_1024_NO_SCALE, ADDITIVE_CIRCULAR_4_2048_NO_SCALE, ADDITIVE_CIRCULAR_6_1024_NO_SCALE,
		CIRCULAR_4_512, CDK_1024;
	}
	...
	public static float[] fingerprints(String smiles, FingerprintsType type) throws CDKException {
		...
		BitSet fp = null;
		if (type == FingerprintsType.ADDITIVE_CIRCULAR_4_2048_NO_SCALE) {
			return (circularAdditiveFingerPrints(mol, CircularFingerprinter.CLASS_ECFP4, 2048, false));
		}
		...
		if (type == FingerprintsType.CDK_1024) {
			Fingerprinter fpGen = new Fingerprinter(1024);
			fp = fpGen.getBitFingerprint(mol).asBitSet();
		}
	...
	public static float[] funcGroups(String smiles) throws CDKException {
		...
		float[] result = new float[patterns.length];
	...
	public static float[] smartsPattersnCounts(String smiles, String[] smartQueries) throws CDKException {
		IAtomContainer mol = smilesToAtomContainer(smiles);
		float[] result = new float[smartQueries.length];

		for (int i = 0; i < smartQueries.length; i++) {
			result[i] = smartsCount(mol, smartQueries[i]);
		}
		return result;
	}
	...
	public static int smartsCount(IAtomContainer mol, String smarts) throws CDKException {
	...
	private static DescriptorValue computeDescriptor(IAtomContainer mol, IMolecularDescriptor md) {
		try {
			DescriptorValue rslt = md.calculate(mol);
			return rslt;
		} catch (Throwable e) {
			e.printStackTrace();
